...
<script type="text/javascript">
	...
	function searchOne(num){
		$.ajax({
			type: 'GET',
			url: 'http://localhost:8090/api/productview/' + num,
			contentType: 'application/json; charset=utf-8',
			success: function(res){
	...
	function insertFunc(JSONObject){
		$.ajax({
			type: 'POST',
			url: 'http://localhost:8090/api/productreg',
			contentType: 'application/json; charset=utf-8',
	...
	function updateFunc(JSONObject){
		$.ajax({
			type: 'PUT',
			url: 'http://localhost:8090/api/productupdate',
			contentType: 'application/json; charset=utf-8',
			data: JSON.stringify(JSONObject),
	        dataType: 'json',
			success: function(res){
	...
	function deleteFunc(num){
		$.ajax({
			type: 'DELETE',
			url: 'http://localhost:8090/api/productdelete/' + num,
			contentType: 'application/json; charset=utf-8',
			success: function(res){
				refresh();
				$("#num, #name, #price").val("");
...
<body>
	...
	<h3>상품 관리</h3>
	<button class="searchListBtn">목록조회</button> <span>(상세 내용을 조회하기 위해서 모록의 아이템을 클릭하세요.)</span>
	<div id="listBox"></div>
